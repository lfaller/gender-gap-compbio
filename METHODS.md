...
We compiled a cohort of 977,731 unique authors from publications indexed in PubMed (2015–2025) retrieved using three controlled vocabulary searches: `"Biology"[Mesh]` for biology articles, `"Computational Biology"[Majr]` for computational biology articles (major topic only), and `"Bioinformatics"[Mesh]` for bioinformatics articles. Following the methodological approach of Bonham & Stefan (2017), we restricted Computational Biology to papers where this is a major MeSH topic to maintain consistency with their original methods. For Bioinformatics, we used the broader [Mesh] tag (rather than [Majr]) to capture the full landscape of computational work on biological data, as [Majr] would yield only ~67,500 papers with 99.5% overlap to the Computational Biology dataset, whereas [Mesh] captures ~225,700 papers with 99% overlap to both Biology and Computational Biology—thereby including ~99,000 additional papers that fall under the broader bioinformatics umbrella. The broader Biology search includes all papers tagged with Biology in any capacity. Publications were queried using the NCBI Entrez API and stored in a SQLite database (gender_data.db) with the following fields: PMID, publication year, journal name, and complete author lists with first and last names. Note that these three search terms overlap substantially: Computational Biology is a 100% subset of the Biology search (as noted by Bonham & Stefan), while Bioinformatics shares significant overlap with Biology. Each unique paper is counted only once in the total, but papers appear in multiple search-based analyses.
